...
var myInfo = { isSupporterConnected: false, isChangingVideo: false };
            ...
            if (myInfo.viewerUsername == "null") {
                ...
                g("#viewernamebt", dialog).onclick = function () {
                    ...
                    if (!check) {
                        ...
                        toast.error("در نام خود از حروف غیر مجاز : " + check[1] + " استفاده شده لطفا اصلاح کنید");
                    }
                }
...
function peerConnectionCallback(id) {
    monsole.log("============== peerConnectionCallback isOn? ============== id:", id);
...
function setTalkState(isSuccessful, j) {
    ...
    if (isSuccessful && j.data.talkNumber) {
        ...
        toast.success("با موفقیت از لیست انتظار مشاوره حذف شدید");
    }
}
function setEndTalk() {
...
function openFavoriteList() {
    ...
    j.control = '<span id="open_favorite_list_bt" class="button_ctr2 bg-css-blue bt">بله نشونم بده</span>' +
